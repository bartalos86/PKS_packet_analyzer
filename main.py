...
def filter_frames_tcp(frames_database, filter, offset=0):
    ...
    for i in range(len(frames_database["packets"])):
        ...
        hexcode = packet["hexa_frame"].strip().replace(
            " ", "").replace("\n", "")
    ...
    for i in range(len(frames_database["packets"])):
        ...
        for pair in found_pairs.values():
             ...
             commm_hash=pair["src_ip"] + pair["dst_ip"] + \
                 str(pair["src_port"]) + str(pair["dst_port"])
             reversed_comm_hash=pair["dst_ip"] + pair["src_ip"] + \
                 str(pair["dst_port"]) + str(pair["src_port"])
...
def filter_frames_udp(frames_database, offset=0):
    ...
    for i in range(len(frames_database["packets"])):
        ...
        src_ip=packet["src_ip"]
        dst_ip=packet["dst_ip"]
        packet_hexcode=packet["hexa_frame"].strip().replace(
            " ", "").replace("\n", "")
...
def analyze_frames(pcap_file=pcap_name, filter="", filter_type=""):
    ...
    ipv4_history={}
    length=len(packets)
    frames_database={"name": "PKS2022/23",
        "pcap_name": pcap_name, "packets": []}
    is_filtering=filter != ""
